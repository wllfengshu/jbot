...
public class TemplateBoot {
    ...
    public void start(String projectName, String packageName, List<Table> tables) throws CustomException {
        ...
        listRoot.forEach(i -> tasks.add(new GeneratorCodeTask(projectName, data, StringUtil.splitFtlSuffix(i), i)));
        //java
        ...
        listJava.forEach(i -> {
            ...
            if ("Application.java.ftl".equals(i)){
                tasks.add(new GeneratorCodeTask(projectName, data, packageProjectPath + projectName4FU + "Application.java", Constant.SRC_MAIN_JAVA_PATH2 + i));
            }else {
                tasks.add(new GeneratorCodeTask(projectName, data, StringUtil.splitFtlSuffix(packageProjectPath + i), Constant.SRC_MAIN_JAVA_PATH2 + i));
            }
        });
        //resources
        List<String> listResources = FileUtil.giveFileName4Dir(Constant.TEMPLATE_NAME + Constant.SRC_MAIN_RESOURCES_PATH);
        listResources.forEach(i -> tasks.add(new GeneratorCodeTask(projectName, data, StringUtil.splitFtlSuffix(Constant.SRC_MAIN_RESOURCES_PATH2 + i), Constant.SRC_MAIN_RESOURCES_PATH2 + i)));
        //table(有多少张表，就循环多少次)
        for (Table table : tables) {
            ...
            tasks.add(new GeneratorCodeTask(projectName, tableData, packageProjectPath + "dao/" + tableName4FUH + "DAO.java", "src/main/java/dao/DAO.java.ftl"));
            tasks.add(new GeneratorCodeTask(projectName, tableData, packageProjectPath + "entity/" + tableName4FUH + "Entity.java", "src/main/java/entity/Entity.java.ftl"));
            tasks.add(new GeneratorCodeTask(projectName, tableData, packageProjectPath + "rest/" + tableName4FUH + "Rest.java", "src/main/java/rest/Rest.java.ftl"));
            tasks.add(new GeneratorCodeTask(projectName, tableData, packageProjectPath + "service/" + tableName4FUH + "Service.java", "src/main/java/service/Service.java.ftl"));
            tasks.add(new GeneratorCodeTask(projectName, tableData, packageProjectPath + "service/impl/" + tableName4FUH + "ServiceImpl.java", "src/main/java/service/impl/ServiceImpl.java.ftl"));
            //mapper
            tasks.add(new GeneratorCodeTask(projectName, tableData, "src/main/resources/mapper/" + tableName4FUH + ".xml", "src/main/resources/mapper/mapper.xml.ftl"));
            //doc
            tasks.add(new GeneratorCodeTask(projectName, tableData, "doc/" + tableName4FUH + "Doc.md", "doc/Doc.md.ftl"));
        }
        CountDownLatch latch = new CountDownLatch(tasks.size());
        tasks.forEach(i -> {
            Future<?> submit = executorService.submit(i);
            while (true) {
                if (submit.isDone()) {
                    latch.countDown();
                    break;
